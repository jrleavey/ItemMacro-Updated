...
export class DND5e extends BaseSystem {
    ...
    registerHooks() {
        // Register a new "macro" activity type
        Hooks.once("dnd5e:initSystemConfig", () => {
            const config = CONFIG.DND5E.itemActionTypes;
            if (!config.macro) {
                config.macro = {
                    label: "Macro",
                    type: "utility"
        ...
        // Only run macros on "macro" activity types
    ...
    preUseActivity(activity, usageConfig, dialogConfig, messageConfig) {
        ...
        // ✅ Only trigger for macro activity type
        const isMacroActivity = activity?.type === "macro";
        if (!isMacroActivity) return true;
